...
/*
 * orderEventProducer (reqSink<OrderEvent> / out: order-events) --> processor (in: order-events / out: payment-events) --> paymentEventConsumer (resSink<PaymentEvent / in: payment-events)
 * processor: Processes order events and produces payment events
 */
public class PaymentServiceTest extends AbstractIntegrationTest {
    ...
    public void deductAndRefundTest(){
        ...
        expectEvent(orderCreatedEvent, PaymentEvent.Deducted.class, e -> {
        ...
        expectEvent(cancelledEvent, PaymentEvent.Refunded.class, e -> {
            Assertions.assertNotNull(e.paymentId());
            Assertions.assertEquals(orderCreatedEvent.orderId(), e.orderId());
            Assertions.assertEquals(6, e.amount());
        });
    ...
    public void customerNotFoundTest(){
        ...
        expectEvent(orderCreatedEvent, PaymentEvent.Declined.class, e -> {
            Assertions.assertEquals(orderCreatedEvent.orderId(), e.orderId());
            Assertions.assertEquals(6, e.amount());
    ...
    public void insufficientBalanceTest(){
        ...
        expectEvent(orderCreatedEvent, PaymentEvent.Declined.class, e -> {
            ...
            Assertions.assertEquals("Insufficient balance", e.message());
        });
    }
